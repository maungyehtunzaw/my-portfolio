...
import { Typography, Card, Tag, Tabs, Image, Row, Col } from 'antd';
...
function Projects() {
          ...
          {filteredProjects.map(project => (
            <Col xs={24} sm={24} md={12} lg={8} key={project.id} style={{ display: 'flex' }}>
              <Card
                ...
                title={<div style={{ padding: '10px',backgroundSize:"100% 100%" }}>{project.title}</div>}
                bordered={true}
  ...
  return (
    <div>
      ...
      <Tabs defaultActiveKey={categories[0]} onChange={handleCategoryChange} items={tabItems} />
    </div>
  );
}
